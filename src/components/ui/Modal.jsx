    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <div className="absolute inset-0 bg-surface-900/60 backdrop-blur-[3px] transition-opacity" />
          <motion.div
            initial={{ y: '100%', opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: '100%', opacity: 0 }}
            transition={{ type: 'spring', damping: 28, stiffness: 300 }}
            className={`relative w-full ${sizes[size]} bg-white dark:bg-surface-900 shadow-2xl ring-1 ring-surface-900/5 dark:ring-white/10 sm:rounded-2xl rounded-t-2xl overflow-hidden max-h-[95vh] sm:max-h-[85vh] flex flex-col`}
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-surface-100 dark:border-surface-800 flex-shrink-0 bg-white/50 dark:bg-surface-900/50 backdrop-blur-sm z-10">
              {/* Mobile drag handle */}
