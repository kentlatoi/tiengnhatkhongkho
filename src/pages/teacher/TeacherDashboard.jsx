...
import { classesStore, sessionsStore, vocabItemsStore, grammarPointsStore, usersStore } from '../../store/localStorage';
import { motion } from 'framer-motion';

export default function TeacherDashboard() {
  const { user } = useAuth();
  const classes = classesStore.getByTeacher(user.id);
  const sessions = sessionsStore.getAll().filter(s => classes.some(c => c.id === s.classId));
  const students = usersStore.getStudents();
  const vocab = vocabItemsStore.getAll();
  const grammar = grammarPointsStore.getAll();
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {stats.map((s, i) => (
          <DashboardCard key={s.title} {...s} delay={i} />
        ))}
      </div>
