  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {students.map((s, i) => {
          const sClasses = classes.filter(c => c.teacherId === s.id || (c.studentIds || []).includes(s.id));
          return (
            <motion.div key={s.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
              ...
              <div className="flex items-center gap-4 text-sm text-surface-500">
                <span>📚 {sClasses.length} lớp</span>
                <span>📅 {s.createdAt || '—'}</span>
              </div>
              {sClasses.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {sClasses.map(c => <span key={c.id} className="badge-primary text-[10px]">{c.name}</span>)}
                </div>
              )}
            </motion.div>
          );
